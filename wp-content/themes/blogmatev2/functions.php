...
final class Blogmate {
	...
	public function __construct() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'blogmate_after_article_markdown' ) );
	}
	...
	function blogmate_enqueue_markdown_scripts() {
		...
		wp_enqueue_script( 'mermaid', '//cdn.jsdelivr.net/npm/mermaid@10.6.1/dist/mermaid.min.js', array(), '11.10.0', true );
	}
	...
	function blogmate_breadcrumb() {
		...
		if (is_category() || is_single()) {
			...
			if ($category) {
				$cat = $category[0];
				echo '<a href="' . get_category_link($cat->term_id) . '">' . $cat->name . '</a>' . $sep;
			}
